...
export function KanbanColumn({ status, tasks, onTaskClick }: KanbanColumnProps) {
  ...
  return (
    ...
    >
      {/* Column header with title + status-colored task count badge */}
      <div className="flex items-center justify-between px-3 py-3">
        <h3 className={cn('text-sm font-semibold', meta.header)}>
          {meta.label}
        </h3>
        <span className={cn('rounded-full px-2 py-0.5 text-xs font-semibold', meta.badge)}>
          {tasks.length}
        </span>
      </div>

      {/* Scrollable task list, min-height 400px */}
      <SortableContext items={tasks.map((t) => t.id)} strategy={verticalListSortingStrategy}>
        <div
          ref={setNodeRef}
          className={cn(
            'flex flex-col gap-2 overflow-y-auto p-2 transition-colors duration-150',
        ...
        >
          ...
          {tasks.length === 0 && (
            <div className="flex flex-1 items-center justify-center py-8 text-xs text-muted-foreground/60 select-none">
              Drop tasks here
            </div>
          )}
        </div>
