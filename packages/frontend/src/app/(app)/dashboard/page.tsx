...
const STAT_CARDS = [
  ...
  },
  {
    ...
    delta: '3 due today',
    icon: Clock,
    borderColor: 'border-l-[hsl(var(--status-in-progress))]',
  ...
  },
  {
    ...
    icon: CheckCircle2,
    borderColor: 'border-l-[hsl(var(--status-done))]',
  ...
  },
  {
    ...
    icon: AlertCircle,
    borderColor: 'border-l-[hsl(var(--status-blocked))]',
    iconClass: 'text-[hsl(var(--status-blocked))] bg-[hsl(var(--status-blocked)/0.1)]',
  },
];

const ACTIVITY = [
  { id: '1', user: 'Alice', action: 'moved', target: 'Design login screen', to: 'In Review', time: '2m ago', status: 'in_review' as const },
  { id: '2', user: 'Bob', action: 'completed', target: 'Set up CI pipeline', to: 'Done', time: '18m ago', status: 'done' as const },
  { id: '3', user: 'Carol', action: 'created', target: 'Add dark mode support', to: 'Backlog', time: '1h ago', status: 'backlog' as const },
  { id: '4', user: 'Dave', action: 'flagged', target: 'API rate limit issue', to: 'Blocked', time: '2h ago', status: 'blocked' as const },
  { id: '5', user: 'Alice', action: 'started', target: 'User profile page', to: 'In Progress', time: '3h ago', status: 'in_progress' as const },
  { id: '6', user: 'Bob', action: 'reviewed', target: 'Payment integration', to: 'In Review', time: '4h ago', status: 'in_review' as const },
  { id: '7', user: 'Carol', action: 'completed', target: 'Write unit tests', to: 'Done', time: '5h ago', status: 'done' as const },
];
...
export default function DashboardPage() {
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Stat cards — colored left border per status */}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Activity feed — scrollable list with timestamp and avatar */}
        <Card className="lg:col-span-2 flex flex-col">
          ...
          <CardContent className="p-0 flex-1 overflow-hidden">
            <ul className="divide-y divide-border overflow-y-auto max-h-72 scrollbar-thin">
              {ACTIVITY.map((item) => (
                <li key={item.id} className="flex items-start gap-4 px-6 py-3">
